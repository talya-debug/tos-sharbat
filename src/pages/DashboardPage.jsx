...
export default function DashboardPage() {
  ...
  return (
    <div>
      ...
      <h2 className="text-[32px] leading-[40px] font-bold text-text-primary mb-1">ניהול מלאכות באתר</h2>
      ...
      <div className="grid grid-cols-4 gap-6 mb-10">
        {stats.map((s, i) => (
          <div key={i} className="bg-white border border-border rounded-xl shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
              <p className="text-[28px] font-bold text-text-primary leading-tight">{s.value}</p>
              <p className="text-text-secondary text-[12px]">{s.label}</p>
            </div>
          </div>
        ))}
      </div>
